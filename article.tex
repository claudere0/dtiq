...
\begin{abstract}
This paper presents a task-aware empirical investigation into the effects of image compression and quantization on downstream deep learning-based computer vision systems. Traditional image compression frameworks have historically been optimized for visual appeal to human observers, using perceptual metrics such as Peak Signal-to-Noise Ratio (PSNR) and Structural Similarity Index Measure (SSIM). However, modern autonomous workflows increasingly rely on neural networks, whose feature-extraction pipelines process visual artifacts differently than the human visual system. To bridge this research gap, we evaluate two distinct image degradation and storage strategies using the standard COCO val2017 dataset (5,000 images) and a YOLOv8n object detection model. The first branch utilizes JPEG recompression at quality levels 94, 88, 75, 50, and 25. The second branch applies uniform RGB bit-depth reduction (BPC) to 7, 4, 3, 2, and 1 bits per channel, stored in a 24-bit truecolor lossless PNG container. Our findings reveal a significant divergence between classical visual quality and downstream task performance. JPEG recompression exhibits remarkable efficiency; quality level 88 achieves 2.02$\times$ storage reduction (halving the dataset size to 384.22 MB) with a negligible relative drop in mean Average Precision (mAP50) of less than 1\% (0.5147 compared to the baseline 0.5187). Conversely, the uniform bit-depth reduction branch yields poor storage-task efficiency when constrained by the tested 24-bit PNG container. While 7-bit quantization preserves baseline object detection performance (0.5189 mAP50), its file size inflates to 1912.67 MB (0.41$\times$ negative compression) due to spatial banding reducing the entropy coding efficiency of the PNG DEFLATE algorithm. Furthermore, BPC levels that achieve storage savings ($b \le 3$) cause a catastrophic drop in detection accuracy ($>15.3\%$ drop at $b=3$, and near total failure at lower bitrates). We conclude that under the evaluated BPC + 24-bit PNG storage scheme, JPEG recompression is vastly more efficient for task-aware machine vision workflows, and we establish that visual quality metrics alone are insufficient predictors of deep learning task stability.
\end{abstract}
...
Ultimately, this work presents a task-aware evaluation framework that challenges the reliance on visual similarity metrics for machine-centric visual pipelines. We demonstrate that JPEG recompression provides a highly efficient and gradual trade-off curve, whereas uniform BPC stored via 24-bit PNG suffers from severe container-level storage inflation and rapid detection degradation at lower bit depths.
...
For the downstream task, we utilized \textbf{YOLOv8n} (the nano variant of the YOLOv8 family) \cite{jocher2023}. The model was loaded with pre-trained weights (\texttt{yolov8n.pt}) and run directly in validation mode. YOLOv8n was selected because it is a widely used lightweight real-time detector whose compact parameter space (3.2 million parameters) makes it highly sensitive to input signal quality, acting as an excellent ``canary in the coal mine'' for detecting structural information loss under severe compression.
...
\begin{itemize}
    ...
    \item \textbf{The Role of Frequency Filtering}: The slight increase in Precision observed at $q88$ (0.6446 vs. baseline 0.6347) may suggest that mild JPEG recompression acts as a spatial regularizer. By quantizing high-frequency DCT coefficients, JPEG can suppress minor pixel-level textures and background details. This provides a plausible explanation for the Precision increase, although confirming the exact mechanism would require additional per-class and false-positive analysis.
\end{itemize}
...
Specifically, JPEG quality factor \textbf{q88} represents the most efficient practical operating point, halving the dataset storage size (2.02$\times$ compression) with less than a 1\% relative drop in object detection accuracy ($mAP50 = 0.5147$ vs. baseline $0.5187$). For tighter budgets, \textbf{q75} provides 3.25$\times$ compression with only a 2.51\% mAP50 drop. Uniform RGB bit-depth reduction stored in 24-bit PNG is highly inefficient: it introduces spatial banding that disrupts the PNG DEFLATE compressor, causing file size inflation at high bit depths ($b7$ is 2.46$\times$ larger than baseline) and collapsing downstream detection accuracy at lower bit depths. Traditional visual quality metrics (PSNR, SSIM) diverge significantly from neural network task metrics (mAP), confirming that human-centric evaluation is insufficient for machine vision engineering.
