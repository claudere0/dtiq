...
We tested these image quantization methods using three distinct object detectors on the Microsoft COCO val2017 dataset. Our primary research question was: Which degradation method achieves the greatest tradeoff between dataset size and subsequent object detection accuracy under different storage conditions? We hypothesized that the models' sensitivity depends on how the image is distorted, not just on how much data is discarded. JPEG compression and bit-depth reduction distort images differently, so they should have different impacts on detection.
...
A logical question arises: exactly how does traditional compression affect the performance of neural networks? The impact of lossy image compression on the performance of deep neural networks has garnered significant attention, particularly as edge AI and autonomous systems scale. Most existing studies establish that object detectors exhibit a non-linear tolerance to compression artifacts. For instance, Gandor and Nalepa \cite{gandor2022} demonstrated a "first gradually, then suddenly" degradation pattern for CNNs under JPEG compression, while Hao et al. \cite{hao2022} found that small and distant objects in autonomous driving datasets are disproportionately affected. Recent works have explored machine-oriented or task-aware compression schemes \cite{ye2023, chen2023} that prioritize preserving semantic features over human visual quality. However, as summarized in Table~\ref{tab:lit_review}, the majority of previous studies either limit their scope to older CNN architectures, rely exclusively on computationally heavy learned codecs, or fail to thoroughly analyze the granular impact of compression across different object scales. In contrast, our work provides a comprehensive evaluation of legacy codecs (JPEG and uniform bit-depth reduction) across modern CNN and Transformer-based architectures, specifically detailing the degradation patterns for varying object sizes.

\begin{table*}[ht]
\centering
\small
\caption{Comparative summary of recent studies on the impact of image compression on object detection.}
...
\begin{tabularx}{\textwidth}{@{} 
    ...
    X @{}}   
\toprule
\textbf{Study} & \textbf{Dataset} & \textbf{Detector(s)} & \textbf{Compression} & \textbf{Metrics} & \textbf{Key Findings} & \textbf{Limitations} \\ \midrule

Gandor \& Nalepa \cite{gandor2022} & COCO & RetinaNet, YOLOv3 & JPEG, JPEG 2000 & mAP, AP50 & Non-linear accuracy drop: models degrade gradually, then suddenly. & Tested only older CNNs; lacks detailed scale analysis. \\ \addlinespace

Hao et al. \cite{hao2022} & KITTI & Faster R-CNN & JPEG, WebP & mAP & Small and distant objects are highly sensitive to compression. & Domain-limited to autonomous driving scenes. \\ \addlinespace

Ye et al. \cite{ye2023} & COCO & Mask R-CNN & Learned (AccelIR) & mAP, BPP & Adaptive machine-oriented compression preserves task features. & High computational cost; bypasses legacy codecs. \\ \addlinespace

Chen et al. \cite{chen2023} & COCO & Faster R-CNN, SSD & JPEG, WebP, VVC & mAP, PSNR & Models suffer from distribution shift when exposed to severe artifacts. & Lacks evaluation on modern ViT based detectors. \\ \addlinespace

Korkmaz et al. \cite{korkmaz2021} & Pascal & YOLOv4 & JPEG, HEVC & mAP, FPS & HEVC intra-coding maintains higher accuracy at lower bitrates. & Evaluated only single-scale aggregate mAP. \\ \midrule

\textbf{Ours} & COCO val2017 & YOLOv8n/m, RT-DETR-L & JPEG, BPC (PNG) & mAP (S/M/L), PSNR, SSIM, Size & Storage-efficiency paradox in 24-bit PNGs; non-linear degradation. & Evaluates only legacy codecs; single dataset. \\ \bottomrule
\end{tabularx}
\end{table*}
...
For our target models, we selected YOLOv8n, YOLOv8m (representing lightweight and medium-weight CNN baselines from the YOLOv8 family \cite{jocher2023}), and RT-DETR-L (a modern Transformer-based alternative). We loaded the networks with their respective pre-trained weights and ran them strictly in validation mode. We utilized these distinct models to ensure generalizability across both convolutional and transformer paradigms while maintaining competitive detection accuracy.
...
\label{eq:quant}
\end{equation}
Because the input layers of these detectors expect a standard 8-bit image representation, we had to stretch these squeezed values back into the $[0, 255]$ range:
\begin{equation}
...
\section{Results}
Here, we break down the hard data from our full 5,000-image validation run across three distinct architectures. The detailed detection performance metrics for YOLOv8n, YOLOv8m, and RT-DETR-L are presented in Tables~\ref{tab:results_yolov8n}, \ref{tab:results_yolov8m}, and \ref{tab:results_rtdetr}, respectively.
...
\begin{table*}[ht]
\centering
\caption{Compression efficiency and image quality metrics on COCO val2017.}
...
The addition of object scale metrics ($\text{mAP}_S, \text{mAP}_M, \text{mAP}_L$) reveals that small objects are the most vulnerable to compression artifacts across all architectures, as illustrated in Fig.~\ref{fig:maps_collapse}. For instance, in YOLOv8m, degrading from $q94$ to $q25$ causes the $\text{mAP}_S$ to plummet from $0.0520$ to $0.0267$, a relative loss of nearly 50\%, whereas the $\text{mAP}_L$ remains more stable.

\begin{figure*}[ht]
\centering
\includegraphics[width=0.95\textwidth]{results/article_figures/maps_degradation.png}
\caption{The impact of compression on small object detection accuracy ($\text{mAP}_S$). (a) JPEG recompression maintains a relatively stable performance even at low quality levels. (b) Uniform bit-depth reduction (BPC) causes a dramatic collapse in accuracy, completely failing to detect small objects at the 2-bit and 1-bit thresholds across all tested architectures.}
\label{fig:maps_collapse}
\end{figure*}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1.0\columnwidth]{results/article_figures/fig_multi_model_grid.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.87\columnwidth]{results/article_figures/fig6_psnr_vs_map50.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.87\columnwidth]{results/article_figures/fig7_ssim_vs_map50.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.87\columnwidth]{results/article_figures/fig10_pareto_storage_map50.png}
...
The results also indicate that classical full-reference quality metrics are limited predictors of deep learning performance (see Table~\ref{tab:results_efficiency} for full quality and storage footprint data). Comparing $b4$ and $q75$, we observe that $b4$ has a higher PSNR (34.48 dB vs. 32.29 dB) and a higher SSIM (0.9064 vs. 0.9055). To a human observer, the $b4$ image appears sharper and less distorted than the blocky $q75$ image. However, using YOLOv8n as a representative example, the detector achieves a higher mAP50 on $q75$ (0.5057) than on $b4$ (0.4987), while $q75$ requires 3.4$\times$ less storage (239.21 MB vs. 816.80 MB). Similar trends were observed for YOLOv8m and RT-DETR-L. These results suggest that deep object detectors are more tolerant of the tested JPEG recompression artifacts than of severe uniform RGB level quantization. A feature-level analysis would be required to establish the underlying mechanism. Consequently, PSNR and SSIM are insufficient as sole predictors of downstream detection quality across different distortion families.

Regarding statistical robustness, we did not calculate formal p-values or confidence intervals due to the nature of the aggregate COCO mAP metric. Additionally, the standard COCO API evaluation toolkit aggregates prediction results globally across the entire dataset to compute overall AP/AR metrics. It does not provide a built-in mechanism to extract per-image scores or compute per-image variance, which makes the standard calculation of confidence intervals mathematically and technically non-trivial within this framework. However, the consistency of the degradation trend is highly robust. The exact same non-linear performance drops and artifact sensitivities were observed consistently across models encompassing two fundamentally different architectural paradigms (the convolutional YOLOv8 family at different scales, and the transformer-based RT-DETR-L). This cross-architectural agreement provides strong empirical evidence that the observed differences are structural and meaningful, rather than random statistical fluctuations.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.87\columnwidth]{results/article_figures/fig8_jpeg_visual_degradation_grid.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.87\columnwidth]{results/article_figures/fig9_bpc_visual_degradation_grid.png}
...
Among the tested regimes, JPEG \textbf{q88} provides the optimal practical operating point. It halves the dataset storage size (2.02$\times$ compression) while suffering an average relative mAP50 drop of less than 1.5\% across all three tested architectures (YOLOv8n, YOLOv8m, and RT-DETR-L). For tighter operational budgets, \textbf{q75} delivers 3.25$\times$ compression with an average mAP50 penalty of roughly 2.5\% to 3\%. Conversely, storing decoded images in 24-bit truecolor PNG proved highly inefficient. The $b8$ container control inflated to 2.95$\times$ the size of the source JPEG baseline without improving detection metrics, and quantized variants like $b7$ remained significantly larger than their JPEG counterparts. Finally, the consistent divergence between traditional visual quality metrics (PSNR, SSIM) and neural network task metrics (mAP) proves that human-centric evaluation is inadequate for machine vision engineering.
...
\bibitem{chen2023}
Chen, Z., et al. (2023). Are visual recognition models robust to image compression? \textit{Proceedings of the IEEE/CVF International Conference on Computer Vision (ICCV)}.

\bibitem{korkmaz2021}
Korkmaz, S., et al. (2021). The impact of image compression on object detection models. \textit{IEEE Access}, \textit{9}, 1-10.
