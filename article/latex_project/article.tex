...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{results/article_figures/fig10_pareto_storage_map50.png}
...
The empirical storage paradox observed in the BPC branch should not be interpreted as evidence that all bit-depth reduction schemes are inherently inefficient. In this study, all BPC variants were encoded using a 24-bit truecolor PNG representation. Consequently, heavily quantized variants such as $b1$, which contain at most $2^3=8$ unique RGB colors, were still stored through a container that represents each pixel using three 8-bit channels before lossless compression.

From a theoretical perspective, this inefficiency could be reduced by using container formats that better match the quantized signal. Indexed PNG, for example, supports paletted representations with 1-, 2-, 4-, and 8-bit indices, which would substantially reduce the raw representation size for extreme BPC modes. More machine-centric pipelines could also use raw bit-packing, storing exactly $b$ bits per channel and then applying a dictionary-based entropy coder such as LZMA. These alternatives are beyond the empirical scope of this paper, but they highlight an important architectural conclusion: for machine vision data pipelines, the physical storage container can be as important as the quantization algorithm itself.
...
\begin{itemize}
    ...
    \item \textbf{BPC Storage Scope}: The bit-depth reduction branch was restricted to uniform RGB quantization saved via a standard 24-bit truecolor lossless PNG container.
