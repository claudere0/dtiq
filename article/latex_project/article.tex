...
\begin{abstract}
We investigate how image storage transformations affect downstream object detection performance on the COCO val2017 dataset using the YOLOv8n detector, focusing on JPEG recompression and RGB bit-depth reduction. Five JPEG recompression levels (94, 88, 75, 50, and 25) were compared with 24-bit PNG storage of decoded RGB images, including an 8-bit PNG control and quantized variants at 7, 4, 3, 2, and 1 bits per channel. All variants were evaluated under identical validation settings using dataset size, Precision, Recall, mAP50, mAP50-95, PSNR, and SSIM. The results show that JPEG recompression offers the best storage-performance trade-off: quality level 88 reduced dataset size by 2.02$\times$ to 384.22 MB while incurring only a 0.77\% relative drop in mAP50 (0.5147 versus 0.5187 for the source JPEG baseline). In contrast, the PNG-based RGB quantization pipeline preserved detection quality at 7 bits per channel (mAP50 = 0.5189) but remained storage-inefficient at 1912.67 MB, and performance degraded sharply at lower bit depths, with $b3$ producing a 15.3\% mAP50 drop and $b2$/$b1$ causing severe failure. These findings indicate that storage representation is a critical design choice for machine-vision pipelines and that JPEG recompression is a more effective practical solution than uniform RGB bit-depth reduction under the tested conditions.
\end{abstract}
