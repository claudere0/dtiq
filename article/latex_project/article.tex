...
\begin{abstract}
This study investigates how image storage transformations affect downstream object detection performance, addressing the gap between classical compression metrics and task-oriented evaluation. Using the COCO val2017 dataset, we systematically compared JPEG recompression at quality levels 94, 88, 75, 50, and 25 with RGB bit-depth reduction stored as 24-bit PNG, including a decoded 8-bit PNG control and quantized variants at 7, 4, 3, 2, and 1 bits per channel. All variants were evaluated with the YOLOv8n detector under identical validation settings, and performance was assessed using dataset size, Precision, Recall, mAP50, mAP50-95, PSNR, and SSIM. The results show that JPEG recompression provides the strongest storage-performance trade-off: quality level 88 reduced dataset size by 2.02$\times$ to 384.22 MB while incurring only a 0.77\% relative drop in mAP50 (0.5147 versus 0.5187 for the source JPEG baseline). In contrast, the PNG-based RGB quantization pipeline preserved detection quality at 7 bits per channel (mAP50 = 0.5189) but remained storage-inefficient at 1912.67 MB, and performance degraded sharply at lower bit depths, with $b3$ producing a 15.3\% mAP50 drop and $b2$/$b1$ causing severe failure. These findings demonstrate that the choice of storage representation is critical for machine-vision pipelines and that JPEG recompression is a more effective practical solution than uniform RGB bit-depth reduction under the tested conditions.
\end{abstract}
