...
All final validation runs were executed in a fixed CPU validation environment:
\begin{itemize}
    \item \textbf{Hardware}: Apple M1 CPU on a MacBook Air with 8 GB unified memory running macOS 15.6.1
    \item \textbf{Software}: Ultralytics YOLOv8 (version 8.4.52) on PyTorch 2.6.0 and torchvision 0.21.0
...
JPEG is a frequency-domain, lossy block-based compression format. The source COCO images (which are already stored as JPEGs) were read and recompressed using the Pillow library in Python. We applied five target quality levels:
...
\begin{itemize}
    \item \textbf{The $b7$ vs $q94$ Comparison}: $b7$ achieves a near-lossless PSNR of 51.22 dB and an SSIM of 0.9980. q94 achieves a similarly high detection accuracy while requiring substantially less storage (37.47 dB PSNR, 0.9677 SSIM).
    \item \textbf{The $b4$ vs $q75$ Comparison}: $b4$ has slightly higher PSNR (34.48 dB vs 32.29 dB) and SSIM (0.9064 vs 0.9055) than $q75$, while $q75$ achieves higher mAP50 and requires substantially less storage.
\end{itemize}
...
\begin{enumerate}
    \item Different types of image degradation affect YOLOv8 in different ways.
...
The BPC branch highlights an important limitation: \textbf{uniform RGB level quantization stored in a standard lossless 24-bit truecolor PNG container is inefficient for storage optimization under the tested pipeline.}
\begin{itemize}
    \item \textbf{Container-Control Result ($b8$)}: Re-encoding the decoded source JPEG as 8-bit truecolor PNG without additional level reduction preserves baseline detection metrics (mAP50 = 0.5187) but increases storage to 2288.66 MB (0.34$\times$ compression relative to the source JPEG baseline). This isolates a substantial container penalty before any additional quantization is applied.
    ...
    \item \textbf{Task Collapse}: At lower bit depths ($b \le 3$), where BPC finally achieves storage savings over the source dataset (e.g., $b3$ at 607.81 MB), severe tonal quantization produces posterized, high-contrast regions and coincides with a sharp decline in detection performance.
\end{itemize}

\subsection{Divergence of Human and Machine Quality}
The results indicate that classical full-reference quality metrics are limited predictors of deep learning performance:
\begin{itemize}
    \item \textbf{Visual vs. Task Fidelity}: Comparing $b4$ and $q75$, we observe that $b4$ has a higher PSNR (34.48 dB vs. 32.29 dB) and a higher SSIM (0.9064 vs. 0.9055). To a human observer, the $b4$ image appears sharper and less distorted than the blocky $q75$ image. However, YOLOv8n achieves a higher mAP50 on $q75$ (0.5057) than on $b4$ (0.4987), while $q75$ requires \textbf{3.4$\times$ less storage} (239.21 MB vs. 816.80 MB).
    \item \textbf{Scientific Implication}: The results suggest that YOLOv8n is more tolerant of the tested JPEG recompression artifacts than of severe RGB level quantization. A feature-level analysis would be required to establish the underlying mechanism. Consequently, PSNR and SSIM are insufficient as sole predictors of downstream detection quality across different distortion families.
\end{itemize}
...
\begin{enumerate}
    ...
    \item JPEG compression and bit-depth reduction at comparable compression levels exert different effects on object detection quality.
...
Although moderate bit-depth reduction preserves information that is meaningful for YOLOv8, under the evaluated data representation scheme it is inferior to JPEG recompression in terms of the combined storage size and detection quality trade-off. The study indicates that the type of information lost is as important as the amount of information lost.
...
\begin{itemize}
    ...
    \item \textbf{Recompression Nature}: The JPEG branch evaluated represents second-order lossy recompression, as the source COCO images were already stored in JPEG format.
    \item \textbf{Hardware Platform}: The final validation was conducted on an Apple M1 MacBook Air CPU with \texttt{batch = 1} and \texttt{workers = 0}. The conclusions pertain to the tested setup and require careful generalization.
\end{itemize}
