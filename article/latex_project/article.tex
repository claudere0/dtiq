...
The Pareto frontier is dominated by JPEG variants from $q88$ through $q25$. No quantized BPC point lies on the frontier: even when BPC achieves smaller files at aggressive quantization levels, its mAP50 remains far below JPEG variants at comparable storage budgets (for example, $b1$ at 165.60 MB vs. $q25$ at 95.94 MB).
...
The PNG paradox motivated a supplementary storage-only experiment. Using the same uniform RGB level quantization as the BPC branch, each of the 5,000 val2017 images was written as an uncompressed 24-bit BMP in memory and then compressed individually with LZMA (preset 6) using the Python \texttt{lzma} module, which implements the Lempel--Ziv--Markov chain algorithm \cite{pavlov2013}. This isolates the effect of the entropy coder from PNG spatial filtering.

Two observations follow immediately. First, the uncompressed BMP payload is effectively independent of the quantization level $b$: every variant still allocates three 8-bit channels per pixel before compression. In our run, each BMP tree occupied approximately 4.1~GB (4,110~MB), matching the expected order of magnitude for 5,000 decoded RGB frames without container-level bit packing. Second, per-file LZMA reduces total size relative to BMP, but it does not uniformly beat PNG.

Table~\ref{tab:lzma_png_storage} compares the official PNG dataset sizes from the main experiment with the auxiliary LZMA archives. At $b7$, LZMA (2005.76~MB) is slightly \textit{larger} than PNG (1912.67~MB), whereas at $b4$--$b1$ LZMA yields 19--32\% smaller archives than PNG while preserving the same quantized pixel values (lossless with respect to the BMP intermediate). JPEG recompression remains far smaller (e.g., $q88$ at 384.22~MB with near-baseline mAP50).

\begin{table}[h]
\centering
\caption{Storage comparison of PNG containers (main experiment) versus per-image BMP+LZMA archives (auxiliary experiment, 5,000 images). Uncompressed BMP size was $\approx$4,110~MB for every tested $b$.}
