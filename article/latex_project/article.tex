...
\begin{abstract}
This paper presents a task-aware empirical investigation into the effects of image compression and quantization on downstream deep learning-based computer vision systems. Traditional image compression frameworks have historically been optimized for visual appeal to human observers, using perceptual metrics such as Peak Signal-to-Noise Ratio (PSNR) and Structural Similarity Index Measure (SSIM). However, modern autonomous workflows increasingly rely on neural networks, whose feature-extraction pipelines process visual artifacts differently than the human visual system. To bridge this research gap, we evaluate two distinct image degradation and storage strategies using the standard COCO val2017 dataset (5,000 images) and a YOLOv8n object detection model. The first branch utilizes JPEG recompression at quality levels 94, 88, 75, 50, and 25. The second branch stores decoded RGB images in 24-bit truecolor lossless PNG, including an $b8$ container control (no additional level reduction) and quantized variants at 7, 4, 3, 2, and 1 bits per channel mapped back to 8-bit RGB. Our findings reveal a significant divergence between classical visual quality and downstream task performance. JPEG recompression exhibits remarkable efficiency; among the tested JPEG settings, quality level 88 achieves 2.02$\times$ storage reduction (halving the dataset size to 384.22 MB) with a negligible relative drop in mean Average Precision (mAP50) of less than 1\% (0.5147 compared to the source JPEG baseline 0.5187). Conversely, the PNG branch is storage-inefficient even before aggressive quantization: the $b8$ control preserves baseline detection (0.5187 mAP50) but increases size to 2288.66 MB (0.34$\times$ compression). Variant $b7$ remains near baseline in detection (0.5189 mAP50) at 1912.67 MB. This storage behavior should be interpreted as the result of the combined interaction between source JPEG decoding, truecolor PNG storage, PNG filtering, DEFLATE entropy coding, and the spatial statistics of quantized RGB images. Furthermore, quantization levels that achieve storage savings ($b \le 3$) cause a severe drop in detection accuracy at $b=3$ ($>15.3\%$ drop), with near-total failure at lower bitrates. We conclude that under the evaluated RGB level quantization + 24-bit PNG storage scheme, JPEG recompression is vastly more efficient for task-aware machine vision workflows, and we establish that visual quality metrics alone are insufficient predictors of deep learning task stability.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Execution Device}: CPU
...
The supplementary storage table below is generated automatically from \texttt{results/article_figures/lzma_storage.csv} via \texttt{scripts/analyze/summarize_lzma_storage.py}, so it can be refreshed whenever the auxiliary experiment is regenerated.

\input{results/article_figures/lzma_storage_table.tex}
...
From a theoretical perspective, this inefficiency could be reduced by using container formats that better match the quantized signal. Indexed PNG supports paletted representations with 1-, 2-, 4-, and 8-bit indices. More machine-centric pipelines could pack exactly $b$ bits per channel and apply dictionary coders such as LZMA \cite{pavlov2013}; Table~\ref{tab:lzma_png_storage} shows that such coding helps relative to raw BMP but is not automatically superior to PNG. The BMP+LZMA experiment does not implement physical $b$-bit packing; it compresses byte-expanded 24-bit RGB representations. Packed low-bit streams remain a subject for future work. The architectural conclusion remains: for machine vision data pipelines, the physical storage container can be as important as the quantization algorithm itself.
